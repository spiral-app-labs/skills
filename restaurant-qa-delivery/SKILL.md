...
After round 3, use `POST`/`PATCH /api/agency/leads/:leadId/build` to update the build stage to `packaging`, then `delivered` only when the pitch doc, battle cards doc, preview URL, screenshots, audit evidence, 30-review packet, top-three-improvement evidence, concierge evidence, checklist paths, and MC requirement/evidence paths are attached.
