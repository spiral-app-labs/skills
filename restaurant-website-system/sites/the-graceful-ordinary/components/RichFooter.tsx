      ...
      <div className="max-w-[1280px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div className="space-y-3">
            <p className="font-display text-accent" style={{ fontSize: '40px', letterSpacing: '0.08em' }}>{c.name}</p>
            <p className="font-display italic text-text" style={{ fontSize: '18px' }}>{f.tagline}</p>
            <p className="text-body-sm text-text/75 leading-relaxed max-w-xs">{f.description}</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2">
              {content.nav.items.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="text-body text-text/75 hover:text-text transition-colors">
                    {item.label}
                  </Link>
                </li>
              ))}
