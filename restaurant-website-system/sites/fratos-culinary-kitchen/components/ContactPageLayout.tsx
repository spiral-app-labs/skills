    ...
    <main className="bg-canvas">
      ...
      <section className="max-w-content mx-auto px-5 md:px-10 pb-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <div>
            <h2 className="text-[28px] leading-[36px] font-extrabold text-ink">Contact paths</h2>
          ...
          <div>
            <h2 className="text-[28px] leading-[36px] font-extrabold text-ink">{c.actionsHeading}</h2>
            <div className="mt-6 space-y-4">
              {c.actions.map((action) => (
                <div key={action.title} className="rounded-card border border-divider p-5 shadow-card">
                  <h3 className="text-body font-extrabold text-ink">{action.title}</h3>
                  <p className="mt-2 text-body-sm text-ink-soft">{action.body}</p>
                  <a
                    href={action.href}
                    className="mt-4 inline-flex items-center justify-center h-10 px-5 rounded-pill bg-accent text-text-on-brand text-button hover:bg-accent-hover transition-colors"
                  >
                    {action.label}
                  </a>
                </div>
