...
export function ConfettiHero() {
  ...
  // Parallax + Ken Burns on the hero photo. Photo wrapper is the scroll target.
  ...
  return (
    <section className="relative overflow-hidden bg-canvas">
      <div className="max-w-content mx-auto px-5 md:px-10 pt-16 md:pt-24 pb-10 md:pb-16">
        ...
        <div className="relative z-10 text-center max-w-[900px] mx-auto">
          ...
          >
            <a
              ...
              className="inline-flex items-center justify-center h-12 px-6 rounded-pill bg-accent text-text-on-brand text-button hover:bg-accent-hover transition-colors w-full sm:w-auto"
            >
              Order Now
            </a>
            <a
              href="/#menu"
              className="inline-flex items-center justify-center h-12 px-6 rounded-pill bg-ink text-text-on-dark text-button hover:bg-ink/80 transition-colors w-full sm:w-auto"
            >
              View Menu
            </a>
          </motion.div>
        ...
        {/* Hero photo — Ken Burns inside, parallax on the outer wrapper */}
        <motion.div
          ref={photoRef}
        ...
        >
          <div className="relative w-full aspect-[4/3] sm:aspect-square overflow-hidden rounded-[36px] sm:rounded-full shadow-card">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={content.hero.photo}
              alt={content.hero.photoAlt}
              className={`absolute inset-0 w-full h-full object-cover ${docVisible ? 'animate-ken-burns' : ''}`}
            />
          </div>
        </motion.div>
      </div>
