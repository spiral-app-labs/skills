...
const target = targets.find(t => t.url.includes('google.com/maps') && (t.url.includes("Mary's+Mexican+Grill") || t.url.includes('Mary%27s+Mexican+Grill') || t.title.includes("Mary's Mexican Grill")));
if (!target) throw new Error("Mary's Mexican Grill Google Maps tab not found");
...
const result = await page.send('Runtime.evaluate', {
  ...
  expression: `(async () => {
    ...
    const parse = txt => {
      ...
      const stopLabels = new Set(['Order type','Meal type','Price per person','Food: 5','Food: 4','Food: 3','Service: 5','Service: 4','Service: 3','Atmosphere: 5','Atmosphere: 4','Atmosphere: 3','Recommended dishes','Recommendation for vegetarians','Vegetarian offerings','Parking space','Parking options','Parking','Noise level','Wait time','Group size','Seating type','Reservation','Dietary restrictions','Special events','Kid-friendliness']);
    ...
    for (let i=0; i<65 && seen.size<35; i++) {
      harvest();
    ...
    return {
      ...
      business: "Mary's Mexican Grill",
      sort: document.body.innerText.includes('sorted from highest rated to lowest rated') ? 'Highest rating' : 'Highest rating (selected/visible list)',
...
const md = ['# Mary\'s Mexican Grill — Google Reviews Highest 30', '', `Captured: ${packet.captured_at}`, `Source: ${packet.source}`, `Sort: ${packet.sort}`, ''].concat(packet.reviews.map((r, i) => `## ${i + 1}. ${r.reviewer}\n\n- Rating: ${r.rating}\n- Date: ${r.date}\n\n${r.text}${r.owner_reply ? `\n\nOwner reply:\n${r.owner_reply}` : ''}\n`)).join('\n');
...
page.close();
console.log(JSON.stringify({ count: packet.reviews.length, total_visible_reviews: packet.total_visible_reviews, sort: packet.sort, screenshot: 'restaurant-website-system/sites/marys-mexican-grill/screenshots/google-reviews-highest-2026-05-04.png' }, null, 2));
