    ...
    <section className="w-full">
      <div className="mx-auto max-w-shell px-5 md:px-10">
        {categories.map((cat, idx) => (
          <div key={cat.title} className={idx === 0 ? 'pb-16 md:pb-24' : 'py-10 md:py-16'}>
              ...
              {cat.items.map((it) => (
                <li key={it.name} className="grid grid-cols-12 gap-4 items-baseline">
                  <div className="col-span-8">
                    ...
                    <p className="text-body text-ink mt-1">{it.desc}</p>
                  </div>
                  <div className="col-span-4 text-right">
                    <span className="text-body text-ink">{it.meta}</span>
                  </div>
                </li>
              ))}
            </ul>
