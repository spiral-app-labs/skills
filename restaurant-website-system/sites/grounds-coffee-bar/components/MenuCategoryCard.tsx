...
export function MenuCategoryCard({ label, items }: Props) {
  return (
    <div className="bg-card rounded-card p-6 md:p-8 border border-divider/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(16,46,36,0.08)]">
      ...
      <ul className="flex flex-col gap-5">
        {items.map((it) => (
          <li key={it.name} className="flex items-baseline gap-4 rounded-photo px-2 py-1 -mx-2 transition-colors duration-200 hover:bg-canvas/70">
