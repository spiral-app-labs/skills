...
- **Warm Garamond at weight 500 (Alinea-class)** — e.g., **Cormorant Garamond weight 500** (Google Fonts, used by `alinea-01`) paired with **EB Garamond weight 400** for body at 1.8× line-height. Single-family two-weight system where display is heavier than body by 100 units — both stay inside the Garamond family. Use for Michelin-caliber light-mode ceremonial fine-dining where prose density + photography carry the persuasion work. Distinct from 1776's Cormorant italic-emphasis approach (which uses italic for structural emphasis) — Alinea-class uses italic only for linguistic emphasis, never structurally.
...
- **Light-mode Michelin-ceremonial (white + black + warm-gray strip)** — confirmed in `alinea-01`. **First light-mode ceremonial palette in catalog.** Canvas pure `#FFFFFF`, text pure `#000000`, bookend strip `#A8A6A1` (warm medium gray — R channel 2-7 above G and B = warm not cool). No saturated accent. Optional secondary `#8C8B88` (darker warm-gray for dividers / mailing-list strip). The only color outside this system is the Tock widget's native navy `#1E3A8A`, deliberately un-customized because it's the platform's brand. Use for world-class Michelin / World's 50 Best restaurants, any ticketed-reservation venue, chef-driven destinations where museum-quiet is the register.
