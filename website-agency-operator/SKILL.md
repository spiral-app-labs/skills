...
1. Call the MC planner first: `GET /api/agency/website-workflow/next?limit=5` with protected MC agency auth (`Authorization: Bearer <MC_SESSION_TOKEN>`) or the MC session cookie. Do not use Supabase service keys or stale agency/webhook tokens as protected agency-route bearer tokens.
2. Work exactly one selected website unless MC explicitly assigns a batch. A single heartbeat may complete adjacent gates for that same website only when MC's `same_heartbeat_continuation` policy allows it, and only after re-querying `/next` after each writeback.
...
10. Before advancing or continuing to another gate in the same heartbeat, re-query `/next` and confirm MC root stage and local checklist stage agree. If they differ, repair through MC API or log a blocker.
...
Every autonomous build advances through 17 canonical child tasks in order, but Evan may package adjacent low-risk gates into one 30-minute heartbeat when MC re-query confirms each transition. Use the exact gate name as root task `metadata.build_stage/currentStage` when possible. Adjacent child tasks are gated by 16 `task_dependencies`, and `tasks.metadata` is canonical for workflow identity, requirements, blockers, and evidence.
...
6. `hero_art_bible` → `hero_art_bible` — generate/sync the personalized hero assets and art bible before the first preview.
7. `template_fork_build` → `building` — fork/build the selected template with real restaurant content and preview evidence.
8. `improvement_pass` → `improving` — run improvement pass after first full fork.
9. `top_three_improvements` → `top_3_improvements` — implement/evidence the top three highest-leverage improvements.
10. `ai_concierge` → `concierge` — truthful restaurant KB, safe handoffs, no fake availability/promises; MC marks the missing dedicated concierge skill as `skill_gap: true`.
11. `pitch_doc` → `pitch` — create/update pitch doc.
12. `battle_cards` → `battle_cards` — create/update objection/demo battle cards.
13. `qa_round_1`, `qa_round_2`, `qa_round_3` — exactly three QA rounds via `/qa-rounds`.
14. `higgsfield_video_handoff` → `ready_for_higgsfield_video` — hand off a QA-clean preview plus hero motion notes for final video/review.
15. `delivery` → `packaging` — package preview URL, screenshots, audit/review evidence, checklist, pitch doc, battle cards, QA rounds, and requirement status.
16. `delivered` — terminal `/build` state only after packaging evidence passes; not a separate child task.
17. `blocked` — temporary blocker state only when the next autonomous gate cannot truthfully advance.

Human review is a separate status category, not a blocker. `ready_for_higgsfield_video` and `final_review` mean OpenClaw has handed the site to Ethan/Higgsfield/final review; do not label that handoff blocked, and do not keep retrying the delivery gate until MC selects it again after human review.
...
Blocked means the next autonomous gate cannot advance after the required skill path and fallback routes were actually tried. Human review, Higgsfield video handoff, and Ethan final review are not blockers.
