...
const OPS_HELPER_SKILLS = new Set([
  'heartbeat-execution',
...
async function main() {
  ...
  for (const dir of skillDirs) {
    if (ACTIVE_AGENCY_SKILLS.has(dir) || REFERENCE_AGENCY_SKILLS.has(dir) || OPS_HELPER_SKILLS.has(dir)) continue
  ...
  console.log(`Validated ${skillDirs.length} root skills (${ACTIVE_AGENCY_SKILLS.size} active agency skills).`)
}
