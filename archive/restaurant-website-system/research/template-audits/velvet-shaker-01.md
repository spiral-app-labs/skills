...
- **Animation / motion:** **Significant — corrected 2026-04-22.** Original audit understated motion ("minimal, intensity 1–2") because the static capture methodology only sampled scroll positions, never observed mid-animation. Re-investigation via DOM inspection of the live site (scripts/_tmp-vs-scroll-capture, see `inspect.json` artifacts) revealed three load-bearing scroll-triggered patterns that drive a substantial part of the template's editorial feel.
  - **Pattern 1 — Shooting-star fill dividers.** Multiple `data-framer-name="Underline"` divs (1px × 1400px, `overflow: hidden`, cream bg at opacity 0.3) sit beneath section labels. On scroll-into-view, an inner element animates `scaleX 0 → 1` from `transformOrigin: left` with an exponential ease (~easeOutExpo, ~1.1s). Reads as a deliberate streak, not a linear sweep. The fire is once-only and not scroll-tied. Adjacent `data-framer-name="Scroll Triger"` markers confirm the trigger pattern. **Recreation: `ScrollFillDivider.tsx`**, used inside `NumberedEyebrow`. `[observed via DOM inspection 2026-04-22]`
  - **Pattern 2 — Curtain image reveals.** Every image on the page sits inside an `overflow: hidden` parent with no clip-path on the image itself. The reveal effect is JS-driven via Framer Motion's `whileInView` — the image's clip-path animates from `inset(100% 0 0 0)` (fully clipped from below) to `inset(0)` (fully visible). The image content itself doesn't translate — what moves is the clip rectangle, opening like a rising curtain. Duration ~1.0s, easeOutQuint. Tile rows use staggered delays so the curtain peels across the row. **Recreation: `CurtainImage.tsx`**, applied to `CocktailPhotoRow`, `MenuPhotoStrip`, `AsymmetricMiniGallery`, `BrandStoryParagraph` thumbnails. `[observed via DOM inspection 2026-04-22]`
  - **Pattern 3 — Sticky image columns.** 5 elements named `data-framer-name="Widget Wrap"` use `position: sticky` to pin image groups while sibling text scrolls past. Most prominent on the brand-story section (long paragraph + 3-thumbnail right column — the column pins). **Recreation: `StickyColumn.tsx`**, applied inside `BrandStoryParagraph`. `[observed via DOM inspection 2026-04-22]`
  - **Pattern 4 — Hero/feature image scale + drift.** `Hero Image` carries `transform: matrix(1.25867, ...)` (1.26x scale) + `willChange: transform` + `overflow: hidden` parent. `Image 02` translated `(-168, 30)`, `Image 03` translated `(0, -381.328)`. These are part Framer-editor offset positioning, part Ken-Burns / parallax-lite on scroll. **Recreation TODO** — current implementation does not yet scale or parallax the hero. Lower priority than patterns 1–3. `[inferred from transform matrices + willChange]`
  - **Page-load reveal:** at t=0.3s only the wordmark + top-right nav are visible — the rest of the hero fades in after. Similar to qitchen's stagger but shorter timeline. `[observed via motion-frame 79679a4d-t0.3s]`
  - **Hover:** `color 0.4s cubic-bezier(0.44, 0, 0.56, 1)` declared on menu + contact pages. `[observed]`
  - **Motion intensity rating (corrected): 3.** Discreet but load-bearing scroll-triggered choreography on every section. Not flashy (no video loops, no cursor effects, no scroll-tied scenes), but the reveals do significant work for the "magazine catalogue" feel.
  - **Methodology note:** the original audit's `shoot-template.mjs` captures static frames at scroll percentages but doesn't dwell long enough to catch animation triggers, and the meta JSON only lists CSS-declared keyframes (Framer Motion uses JS-driven `transform` updates that don't appear there). Future audits should run the DOM-inspect probe (overflow-hidden + position-sticky + transform matrices + Framer name attributes) to catch this.
