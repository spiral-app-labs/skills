...
import { LiveMusicStrip } from './LiveMusicStrip';
import { OwnerQuotePanel } from './OwnerQuotePanel';
...
export function ScrollableHomePage() {
  ...
  return (
    <>
      ...
      <section className="relative mx-auto w-full max-w-shell px-4 pt-4 md:px-6 md:pt-6">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-12">
          {/* BIG LEFT CARD \u2014 main photo (Ken Burns + parallax via HeroPhoto)
              + testimonial overlay */}
          <div id="book" className="relative md:overflow-hidden md:rounded-card md:col-span-7 md:row-span-2">
            <div className="relative aspect-[4/5] overflow-hidden rounded-card md:aspect-auto md:h-full md:min-h-[640px] md:rounded-none">
              <HeroPhoto
      ...
      {/* PRESS STRIP \u2014 DiBenedetto improvement #3: aggregated review scores
          marquee. No press hits found via WebSearch, so platform scores
          stand in. */}
      <PressStrip />

      {/* LIVE MUSIC \u2014 DiBenedetto improvement #1: surfaces Wed/Fri/Sat
          music nights with computed dates + reserve CTA. */}
      <LiveMusicStrip />

      {/* STORY \u2014 chef-owner Vittorio + room */}
      <ScrollRevealStandard as="section" className="mx-auto max-w-shell px-4 pt-24 md:px-6">
      ...
      {/* OWNER QUOTE \u2014 DiBenedetto improvement #2: Vittorio's "Mille grazie"
          sign-off as a load-bearing visual moment. */}
      <OwnerQuotePanel />

      {/* REVIEWS \u2014 horizontal carousel of verbatim review pull-quotes */}
      ...
      <ScrollRevealStandard as="section" className="mx-auto max-w-shell px-4 pt-24 md:px-6">
        <div className="grid grid-cols-1 overflow-hidden rounded-card bg-surface md:grid-cols-12">
          <div className="p-6 md:col-span-5 md:p-10">
            ...
            <h2 className="mt-3 font-display text-manifesto text-ink">
              Forest View Plaza, on Algonquin Road.
            </h2>
