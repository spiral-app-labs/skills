...
const stateColor: Record<OpenStatus['state'], string> = {
  ...
  closed: '#4B5563',        // gray-600, readable on the pale status tint
...
export function LiveOpenStatus({
  ...
  if (variant === 'pill') {
    return (
      <span
        className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border text-sm font-semibold ${className}`}
