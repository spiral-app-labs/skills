// NamedItemGrid — menu-page category-grouped priced list.
// Massive 102px category headers + text-only rows with right-aligned price.
// PROMOTION CANDIDATE — reusable across wine / cocktail / tasting-menu briefs.
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-10 md:gap-x-16 gap-y-8">
              {cat.items.map((it) => (
                <li key={it.name} className="grid grid-cols-12 gap-4 items-baseline">
                  <div className={it.price ? 'col-span-8' : 'col-span-12'}>
                    <h3 className="text-h3">{it.name}</h3>
                    <p className="text-body text-ink/70 mt-1">{it.desc}</p>
                  </div>
                  {it.price && (
                    <div className="col-span-4 text-right">
                      <span className="text-h3 text-ink">{it.price}</span>
                    </div>
                  )}
                </li>
              ))}
