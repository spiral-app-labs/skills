...
export function LiveMapEmbed({
  ...
  return (
    <div
      className={`relative w-full overflow-hidden rounded-lg border border-text-dark/10 bg-[#e8e2d3] ${className}`}
    ...
    >
      ...
      />
      <div className="pointer-events-none absolute left-4 top-4 max-w-[260px] rounded-button bg-bg-cream/95 px-4 py-3 shadow-sm">
        <p className="text-address text-text-dark">{mapLabel || 'Visit us'}</p>
        <p className="mt-1 text-body-sm text-text-muted">{address}</p>
      </div>
      {!hideCta && (
        <a
          href={directionsHref}
