...
for (const [pageName, route] of routes) {
  ...
  for (const [viewportName, width, height] of viewports) {
    ...
    await writeFile(`screenshots/qa3-cdp-${pageName}-${viewportName}-${stamp}.png`, Buffer.from(screenshot.result.data, 'base64'));

    const dom = await client.send('Runtime.evaluate', {
      returnByValue: true,
      expression: 'document.documentElement.outerHTML',
    });
    await writeFile(`scrapes/qa3-cdp-${pageName}-${viewportName}-${stamp}.html`, dom.result.result.value);
  }

  client.close();
}

await writeFile(`scrapes/qa3-responsive-metrics-${stamp}.json`, JSON.stringify(metrics, null, 2) + '\n');
chrome.kill();
try {
