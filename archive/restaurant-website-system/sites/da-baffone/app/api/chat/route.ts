...
import { RESERVATION_INTENT_TOOL, SYSTEM_PROMPT } from '../../../lib/concierge-kb';
import {
  logConciergeMessage,
        ...
        for await (const event of claudeStream) {
          ...
          if (event.type === 'content_block_delta') {
            if (event.delta.type === 'text_delta') {
              ...
              const buffer = toolBuffers[event.index];
              if (buffer) buffer.jsonStr += event.delta.partial_json;
            }
            continue;
          }
          ...
          if (event.type === 'content_block_stop') {
            ...
            if (buffer) {
              ...
              let input: unknown = {};
              try {
                input = buffer.jsonStr.trim() === '' ? {} : JSON.parse(buffer.jsonStr);
              } catch {
                input = null;
              }
              if (input !== null) {
                controller.enqueue(
                  encoder.encode(
                    `data: ${JSON.stringify({
                      tool_use: { name: buffer.name, input },
                    })}\n\n`,
                  ),
                );
              }
