...
// Canonical minimalist footer. PROMOTION CANDIDATE.
// Sits right above the WordmarkEcho.
      ...
      <div className="mx-auto max-w-shell px-5 md:px-10 pt-16 md:pt-24">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-12">
          ...
          <div>
            ...
            <div className="text-body">{contact.bookNow.viaEmail.value}</div>
            <div className="mt-4 text-body text-ink/80">{contact.bookNow.whatsapp.label}</div>
            <div className="text-body">{contact.bookNow.whatsapp.value}</div>
          </div>
        ...
        <div className="vs-divider mt-16 md:mt-20 pt-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <nav className="flex flex-wrap gap-4 md:gap-6">
            {content.footer.links.map((l) => (
              <Link key={l.href} href={l.href} className="vs-link text-micro lowercase">
                {l.label}
              </Link>
            ))}
          </nav>
