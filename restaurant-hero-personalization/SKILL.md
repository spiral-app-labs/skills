...
15. Mission Control records this as the `human_review` status category and enqueues the mobile push notification from that status change. Evan does not create a separate notification task and must not label the handoff blocked.
16. Ethan or the Higgsfield MCP supplies the final `hero_video_url` (or records a truthful no-video exception), then Ethan moves the build to `final_review`, which is still human review until delivery approval.
...
4. **MC env vars** (for the writeback API):
   ...
   - `MC_SESSION_TOKEN` — bearer token for protected Mission Control agency routes
...
If an early prerequisite is missing, surface a single combined blocker and stop. Missing Higgsfield access is not an early blocker and is not a blocker at handoff; `ready_for_higgsfield_video` is the intentional human-review state for Ethan/Higgsfield.
...
- End handoff: `ready_for_higgsfield_video` is written only after the site is OpenClaw-complete and ready for Ethan's video/final review; this is human review, not blocked
